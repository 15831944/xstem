...
tsp::tsp(const std::vector<RS_Vector> &_pointArray)
{
    num_cities = _pointArray.size();
    original_list.clear();

    for(int i=0;i<num_cities;++i)
    {
        original_list.push_back(new city(i, _pointArray[i].x, _pointArray[i].y, i, false));
    }
}
...
int tsp::get_solution_num(int i)
{
    return solution.at(i)->get_id();
}
