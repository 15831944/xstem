...
Graph::Graph(const std::vector<RS_Vector> &_pointArray)
{
    ...
    {
        for(int j=i+1;j<m_nVertexNum;++j)
        {
            start_vertex=i;
            end_vertex=j;
            edge_weight=_pointArray[i].distanceTo(_pointArray[j]);
            ...
            ///3.2.1 construct the vertices
            start_vertex_pt = get_vertex(start_vertex);
            end_vertex_pt = get_vertex(end_vertex);
