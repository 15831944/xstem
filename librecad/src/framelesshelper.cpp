...
void WidgetData::setMax(bool bMax,int x,int y)
{
    if(m_bMax != bMax)
    {
        if(bMax)
...
//        qDebug()<<m_pWidget->width()<<"xxzzxxz";
        m_ptDragPos = event->globalPos() - frameRect.topLeft();
        m_ptDragPos = QPoint((event->x()/(double)m_pWidget->width())*m_rect.width(),event->y());
...
{
    if(m_bMax&&m_bLeftButtonPressed)
    {
        if(event->y()<=30){
            setMax(false);
            qDebug()<<"xzxzxxzz"<<m_pWidget->x()<<m_pWidget->y();
        }else{
            return;
        }
    }
