...
RS_BlockData QG_DialogFactory::requestNewBlockDialog(RS_BlockList* blockList) {
    ...
    if (dlg.exec()) {
       ...
       // ret = RS_BlockData("1", RS_Vector(0.0,0.0), false);
    }
    return ret;
}
...
RS_BlockData QG_DialogFactory::requestNewGroupDialog(RS_BlockList* blockList) {
    //RS_Block* block = nullptr;
    RS_BlockData ret;
    ret = RS_BlockData("", RS_Vector(false), false);
    ...
    QString GroupNumber = QString::number(number);
    ret = RS_BlockData(GroupNumber, RS_Vector(0.0,0.0), false);
    number++;
    return ret;
}
 ...
 */
bool QG_DialogFactory::requestScaleDialog(RS_EntityContainer*container,RS_ScaleData& data) {

qDebug()<<"requestscaledialog";
    QG_DlgScale dlg(container,parent);
    dlg.setData(&data);
    if (dlg.exec()) {
        dlg.updateData();
        return true;
