    ...
    : QWidget(parent, fl)
{
    ...
    lCoord1b->setText("");
    lCoord2b->setText("");
//    this->line1->setStyleSheet("border-left:50px solid black");
//    this->line1->setObjectName("line1");
//    this->line1->setStyleSheet("QFrame#line1{color:#707070}");
//    this->setStyleSheet("QG_CoordinateWidget{border-left:10px solid black}");
    this->pushButton->setStyleSheet("border-left:8px solid #00FFFF;border-right:8px solid transparent;border-top:8px solid #4D4D4D;border-bottom:8px solid #4D4D4D;");
    connect(this->pushButton,SIGNAL(clicked()),this,SLOT(buttonClickSlot()));

    this->label_3->setStyleSheet("border-left:5px solid black;background-color:transparent");
    this->lCoord1->setStyleSheet("font-size:12px");
    this->lCoord2->setStyleSheet("font-size:12px");
    this->lCoord1b->setStyleSheet("font-size:12px");
    this->lCoord2b->setStyleSheet("font-size:12px");
    graphic = NULL;
    prec = 4;
    format = RS2::Decimal;
    aprec = 2;
...
void QG_CoordinateWidget::buttonClickSlot()
{
    qDebug()<<"niahoxxxx....";
//    if(buttonclick==true){
//        this->pushButton->setStyleSheet("border-left:8px solid transparent;border-right:8px solid #00FFFF;border-top:8px solid #4D4D4D;border-bottom:8px solid #4D4D4D;");
//        buttonclick = false;
//    }else{
//        this->pushButton->setStyleSheet("border-left:8px solid #00FFFF;border-right:8px solid transparent;border-top:8px solid #4D4D4D;border-bottom:8px solid #4D4D4D;");
//        buttonclick = true;
