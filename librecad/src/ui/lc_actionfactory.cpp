...
//    action->setIcon(QIcon(":/icons/zoom_pan.svg"));
    ...
    action = new QAction(tr("点"), agm->other);
//    action->setIcon(QIcon(":/icons/points.svg"));

//    action->setIconText("点");
    ...
    action = new QAction(tr("  两 点"), agm->line);
    ...
    action = new QAction(tr("  角 度"), agm->line);
    ...
    action = new QAction(tr("  水 平"), agm->line);
    ...
    action = new QAction(tr("  垂 直"), agm->line);
...
//    action->setIcon(QIcon(":/icons/设置引线.svg"));
...
//     action->setIcon(QIcon(":/icons/区分内外模.svg"));
    //connect();   后期补充
...
//     action->setIcon(QIcon(":/icons/自动微连.svg"));
    //connect();   后期补充
...
//     action->setIcon(QIcon(":/icons/冷却点.svg"));
    //connect();   后期补充
...
//     action->setIcon(QIcon(":/icons/清除冷却点.svg"));
    //connect();   后期补充
...
//     action->setIcon(QIcon(":/icons/炸开微连.svg"));
    //connect();   后期补充
    ...
    action = new QAction(tr("logo"),agm->sort);
     action->setIcon(QIcon(":/icons/机器人.svg"));
    //connect();   后期补充
    ...
    //     action->setIcon(QIcon(":/icons/直线飞行切割.svg"));
        ...
        action = new QAction(tr("ArcFlyingCutting"),agm->flyingcutting);
         action->setIcon(QIcon(":/icons/圆弧飞行切割.svg"));
    ...
    //     action->setIcon(QIcon(":/icons/输出.svg"));
        //connect();   后期补充
...
//    action->setIconText(" 填充");
    ...
    action = new QAction(tr("&Aligned"), agm->dimension);   //改为绘图菜单的对齐，功能改变
//    action->setIcon(QIcon(":/icons/dim_aligned.png"));
    action->setIconText("标注");
    ...
    //平移
    action = new QAction(tr("&Move / Copy"), agm->modify);   //改为绘图菜单里的平移
...
//    action->setIcon(QIcon(":/icons/delete.svg"));
    ...
    action = new QAction(tr("反转"), agm->modify);
    action->setIcon(QIcon(":/icons/revert_direction.png"));
    action->setIconText("反转");
...
*/
    action = new QAction(tr("镜像"), agm->modify);
    action->setIcon(QIcon(":/icons/mirror.png"));
    action->setIconText("镜像");
    connect(action, SIGNAL(triggered()),
    ...
    action = new QAction(tr("偏移"),agm->modify);
    action->setIcon(QIcon(":/icons/offset.png"));
    action->setIconText("偏移");
    connect(action, SIGNAL(triggered()),
...
//    action = new QAction(tr("&Divide"), agm->modify);    //更改为曲线分割
...
//    a_map["ModifyCut"] = action;
    ...
    action = new QAction(tr("拉伸"), agm->modify);
    action->setIcon(QIcon(":/icons/stretch.png"));
    action->setIconText("拉伸");
    connect(action, SIGNAL(triggered()),
...
//    action = new QAction(tr("Ex&plode"), agm->modify);   拆分变为炸开图形
...
//    a_map["InfoArea"] = action;
    ...
    connect(action, SIGNAL(triggered()),action_handler, SLOT(slotBlocksCreate()));   //跳转坐标
...
//    action->setIcon(QIcon(":/icons/settings.svg"));
    ...
    /*尺寸*/
    action = new QAction(tr("缩放"), agm->modify);
    action->setIcon(QIcon(":/icons/scale.png"));
    action->setIconText("缩放");
...
//    action->setIcon(QIcon(":/icons/delete.png"));
    ...
    /*  零件 */
    //导入零件
    action = new QAction(tr("导入"), agm->widgets);
    ...
    action = new QAction(tr("添加到零件库"), agm->widgets);
//    action->setIcon(QIcon::fromTheme("zoom-in", QIcon(":/icons/open.svg")));
...
//    action->setIcon(QIcon(":/icons/导入零件.svg"));
    action->setCheckable(true);

//    m_fileOpen = new ss_OpenFilePre((QC_ApplicationWindow *)(this->parent()));
//    m_fileOpen->file_dialog->show();
//    m_fileOpen->show();
//    connect(action, SIGNAL(toggled(bool)),main_window, SLOT(toggleLeftDockArea(bool)));  //零件库的槽函数
    connect(action, SIGNAL(triggered()), main_window, SLOT(slotPartFileOpen()));       //导入零件时的文件槽函数
 //   action->setCheckable(true);
...
*/
 ...
 //    connect(action, SIGNAL(triggered()),action_handler, SLOT(slotSelectAll()));
     ...
     action->setIcon(QIcon(":/icons/全不选.svg"));
 //    connect(action, SIGNAL(triggered()),action_handler, SLOT(slotSelectAll()));
     ...
     action->setIcon(QIcon(":/icons/反选.svg"));
 //    connect(action, SIGNAL(triggered()),action_handler, SLOT(slotSelectAll()));
 ...
 //    connect(action, SIGNAL(triggered()),action_handler, SLOT(slotSelectAll()));
    ...
    action->setIcon(QIcon(":/icons/导入标准零件.svg"));
    /*connect(action, SIGNAL(triggered()), action_handler, SLOT(slotDeselectAll()));*/  //槽函数暂且未实现
...
//    action->setIcon(QIcon(":/icons/删除所有排样零件.svg"));
    /*connect(action, SIGNAL(triggered()), action_handler, SLOT(slotDeselectAll()));*/  //槽函数暂且未实现
...
//    action->setIcon(QIcon(":/icons/设置板材.svg"));
    /*connect(action, SIGNAL(triggered()), action_handler, SLOT(slotDeselectAll()));*/  //槽函数暂且未实现
...
//    action->setIcon(QIcon(":/icons/删除排样板材.svg"));
    /*connect(action, SIGNAL(triggered()), action_handler, SLOT(slotDeselectAll()));*/  //槽函数暂且未实现
    ...
    /*   排样  */
    //自动排样
    action = new QAction(tr("排样"), agm->widgets);
...
/*
    action = new QAction(tr("排序"), agm->sinsun);
      ...
      action = new QAction(tr("冷却点"), agm->sinsun);
      action->setIcon(QIcon::fromTheme("zoom-in", QIcon(":/icons/move_copy.svg")));
      connect(action, SIGNAL(triggered()), action_handler, SLOT(slotLengQue()));
      action->setObjectName("ExportGCode");
      a_map["LengQue"] = action;

      action = new QAction(tr("模拟加工"), agm->sinsun);
      action->setIcon(QIcon::fromTheme("zoom-in", QIcon(":/icons/move_copy.svg")));
...
//    action->setIcon(QIcon(":/icons/清除排样结果.svg"));
    ...
    connect(action, SIGNAL(triggered()),action_handler, SLOT(slotSelectUnclosed()));  //选择不封闭图形
    ...
    connect(action, SIGNAL(triggered()),action_handler, SLOT(slotSelectExternal()));   //选择所有外模图形
...
//    connect(action, SIGNAL(triggered()),action_handler, SLOT(slotSelectLayer()));   //槽函数未实现
    ...
    connect(action, SIGNAL(triggered()),action_handler, SLOT(slotSelectSmallSize()));   //选择小于指定尺寸的图形
...
//    action->setIcon(QIcon(":/icons/定义群组.svg"));
    connect(action, SIGNAL(triggered()), action_handler, SLOT(slotDefineGroup()));  //群组
    action->setObjectName("DefineGroups");
    a_map["DefineGroups"] = action;

    action = new QAction(tr("&Break Up Selected Groups"), agm->groups);
//    action->setIcon(QIcon(":/icons/打散选中群组.svg"));
    /*connect(action, SIGNAL(triggered()), action_handler, SLOT(slotBlocksEdit()));*/      //打散选中群组
...
//    action->setIcon(QIcon(":/icons/打散所有群组.svg"));
    connect(action, SIGNAL(triggered()), action_handler, SLOT(slotBreakAllGroup()));  //打散所有群组
...
//    action->setIcon(QIcon(":/icons/矩阵排列.svg"));
    connect(action, SIGNAL(triggered()), action_handler, SLOT(slotRectArray()));  //矩形阵列
...
//    action->setIcon(QIcon(":/icons/交互排列.svg"));
    connect(action, SIGNAL(triggered()), action_handler, SLOT(slotInteractive()));  //交互排列
    action->setObjectName("InteractiveArrangement");
    a_map["InteractiveArrangement"] = action;

    action = new QAction(tr("&Full Of"), agm->array);
//    action->setIcon(QIcon(":/icons/布满.svg"));
    /*connect(action, SIGNAL(triggered()), action_handler, SLOT(slotDeselectAll()));*/  //槽函数暂且未实现
    ...
    //优化
    action = new QAction(tr("&Divide"), agm->optimization);    //更改为曲线分割
//    action->setIcon(QIcon(":/icons/divide.svg"));
    connect(action, SIGNAL(triggered()),
            action_handler, SLOT(slotModifyCut()));
    action->setObjectName("ModifyCut");
...
//    action->setIcon(QIcon(":/icons/去除重复线.svg"));
    connect(action, SIGNAL(triggered()), action_handler, SLOT(slotDeleteRepeat()));  //去除重复线
...
//    action->setIcon(QIcon(":/icons/去除小图形.svg"));
...
//    action->setIcon(QIcon(":/icons/合并相连线.svg"));
   connect(action, SIGNAL(triggered()), action_handler, SLOT(slotMerge()));
    ...
    //排序
    action = new QAction(tr("&Grid Sorting"), agm->sort);    //网格排序
...
//    action->setIcon(QIcon(":/distribution/0.png"));
    ...
    action = new QAction(tr("Local &Shortest Space Shift"), agm->sort);    //局部最短空移
    action->setIcon(QIcon(":/distribution/7.png"));
//    connect(action, SIGNAL(triggered()),
//            action_handler, SLOT(slotModifyCut()));   //槽函数暂未实现
    action->setObjectName("LocalShortestSpaceShift");
    a_map["LocalShortestSpaceShift"] = action;

    action = new QAction(tr("&Tool Die Sequencing"), agm->sort);    //刀模排序
...
//            action_handler, SLOT(slotModifyCut()));   //槽函数暂未实现
    ...
    action = new QAction(tr("&Small Picture First"), agm->sort);    //小图优先
...
//            action_handler, SLOT(slotModifyCut()));   //槽函数暂未实现
    ...
    action = new QAction(tr("From The &Inside Out"), agm->sort);    //从内到外
...
//            action_handler, SLOT(slotModifyCut()));   //槽函数暂未实现
    ...
    action = new QAction(tr("From &Left To Right"), agm->sort);    //从左到右
...
//            action_handler, SLOT(slotModifyCut()));   //槽函数暂未实现
    ...
    action = new QAction(tr("From &Right To Left"), agm->sort);    //从右到左
...
//            action_handler, SLOT(slotModifyCut()));   //槽函数暂未实现
    ...
    action = new QAction(tr("From &Top To Bottom"), agm->sort);    //从上到下
...
//            action_handler, SLOT(slotModifyCut()));   //槽函数暂未实现
    ...
    action = new QAction(tr("From &Bottom To Top"), agm->sort);    //从下到上
...
//            action_handler, SLOT(slotModifyCut()));   //槽函数暂未实现
    ...
    action = new QAction(tr("&Clockwise"), agm->sort);    //顺时针
...
//            action_handler, SLOT(slotModifyCut()));   //槽函数暂未实现
    ...
    action = new QAction(tr("&anti-clockwise"), agm->sort);    //逆时针
...
//            action_handler, SLOT(slotModifyCut()));   //槽函数暂未实现
    ...
    action = new QAction(tr("&Prevent Sorting From Changing Direction"), agm->sort);    //禁止排序改变方向
//    action->setIcon(QIcon(":/icons/禁止排序改变方向.svg"));
//    connect(action, SIGNAL(triggered()),
//            action_handler, SLOT(slotModifyCut()));   //槽函数暂未实现
    action->setObjectName("PreventSortingFromChangingDirection");
    a_map["PreventSortingFromChangingDirection"] = action;

    action = new QAction(tr("&Inner Outer Model Of Sorting Time Zone"), agm->sort);    //排序时区分内外模
//    action->setIcon(QIcon(":/icons/排序时区分内外模.svg"));
//    connect(action, SIGNAL(triggered()),
//            action_handler, SLOT(slotModifyCut()));   //槽函数暂未实现
    ...
    action = new QAction(tr("&The Outermost Layer Is Incised"), agm->sort);    //最外层为阴切
//    action->setIcon(QIcon(":/icons/最外层为阴切.svg"));
//    connect(action, SIGNAL(triggered()),
//            action_handler, SLOT(slotModifyCut()));   //槽函数暂未实现
    action->setObjectName("TheOutermostLayerIsIncised");
    a_map["TheOutermostLayerIsIncised"] = action;
    //共边
    action = new QAction(tr("&Common Edge"), agm->layout);    //共边
//    action->setIcon(QIcon(":/icons/共边.svg"));
    //    connect(action, SIGNAL(triggered()),
    //            action_handler, SLOT(slotModifyCut()));   //槽函数暂未实现
    ...
    //桥接
    action = new QAction(tr("&Bridging"), agm->layout);    //共边
//    action->setIcon(QIcon(":/icons/桥接.svg"));
    //    connect(action, SIGNAL(triggered()),
    //            action_handler, SLOT(slotModifyCut()));   //槽函数暂未实现
...
//        action->setIcon(QIcon(":/icons/zoom_window.svg"));
    ...
    action = new QAction(tr("新     建"), agm->file);
//    if (using_theme)
...
//        action->setIcon(QIcon(":/icons/new.png"));
    ...
    action = new QAction(tr("从模板新建"), agm->file);
    if (using_theme)
    ...
    action = new QAction(tr("打     开"), agm->file);
//    if (using_theme)
...
//        action->setIcon(QIcon(":/icons/open.png"));
  ...
  //  connect(action, SIGNAL(triggered()), action_handler, SLOT(slotFileOpenPreview()));
    ...
    action = new QAction(tr("保     存"), agm->file);
    if (using_theme)
    ...
    action = new QAction(tr("另     存"), agm->file);
    if (using_theme)
    ...
    action = new QAction(tr("另 存 全 部"), agm->file);
