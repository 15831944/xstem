...
RS_GraphicView::RS_GraphicView(QWidget* parent, Qt::WindowFlags f)
    ...
    ,panning(false)
{
//    this->setStyleSheet("background-color:red;border:0");
//    this->setBorders(100,100,100,100);
    ...
    //setSelectedColor(QColor(RS_SETTINGS->readEntry("/select", Colors::select)));//selectedColorChange
    setSelectedColor(QColor("yellow"));
...
//    qDebug()<<pen.getColor().red()<<pen.getColor().green()<<pen.getColor().blue();
    int w = pen.getWidth();
    if (w<0) {
        w = 0;
    }
...
//pen.setColor(foreground);
    ...
    if((pen.getColor().stripFlags()==RS_Color(0,0,0))&&background.stripFlags()==RS_Color(1,22,46)){
        pen.setColor(foreground);
    }

//        pen.setColor(foreground);//修改
