    ...
    for (int num=1;
		...
		for(auto e: *container){
			if (e && e->isSelected()) {
                ...
                ec->move(data.offset*num);       //矩阵的多重复制在此处
...
bool RS_Modification::cut(const RS_Vector& cutCoord,
    ...
    default:
        ...
        cut1->trimEndpoint(cutCoord);   //将实体的结束点修剪到指定位置
...
int RS_Modification::explodeNumber()
{
    return addList.size();
}
...
bool RS_Modification::explodeGroup(const bool remove/*= true*/)
{
